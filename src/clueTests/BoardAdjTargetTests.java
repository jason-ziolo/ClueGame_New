...
public class BoardAdjTargetTests {
	...
	public void testAdjacencyDoorways()
	{
		...
		assertTrue(testList.contains(board.getCellAt(15, 0))); // (same, -1)
		assertTrue(testList.contains(board.getCellAt(15, 2))); // (same, +1)
		assertTrue(testList.contains(board.getCellAt(14, 1))); // (-1, same)
		assertTrue(testList.contains(board.getCellAt(16, 1))); // (+1, same)
		assertEquals(4, testList.size());
	}
	...
	public void testTargetsIntoRoom()
	{
		...
		assertTrue(targets.contains(board.getCellAt(17, 13))); // (same, -2)
		
		
		// directly up and down
		board.calcTargets(6, 8, 2);
		targets = board.getTargets();
		assertTrue(targets.contains(board.getCellAt(4, 8))); // (-2, same)
		
		board.calcTargets(6, 20, 2);
		targets = board.getTargets();
		assertTrue(targets.contains(board.getCellAt(7, 20))); // (+1, same)
		
		
		// one up/down, one left/right
		board.calcTargets(5, 4, 3);
		targets = board.getTargets();
		assertTrue(targets.contains(board.getCellAt(4, 3))); // (-1, -1)
		
		board.calcTargets(5, 19, 2);
		targets = board.getTargets();
		assertTrue(targets.contains(board.getCellAt(4, 20))); // (-1, +1)
		
		board.calcTargets(8, 16, 2);
		targets = board.getTargets();
		assertTrue(targets.contains(board.getCellAt(9, 17))); // (+1, +1)
		
		board.calcTargets(15, 2, 2);
		targets = board.getTargets();
		assertTrue(targets.contains(board.getCellAt(16, 1))); // (+1, -1)
	}
